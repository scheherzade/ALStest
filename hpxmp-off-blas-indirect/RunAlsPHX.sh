	...
	do
	   for th in "${thr[@]}"
		...
		export OMP_NUM_THREADS=1
	    	${phylanx_dir}/bin/als_csv_instrumented -i --hpx:threads=${th} --data_csv=/phylanx-data/CSV/MovieLens_20m.csv  --hpx:bind=balanced --hpx:numa-sensitive --iterations=${it} --f=${f} --row_stop=${rs} --col_stop=${cs} --hpx:print-counter=/threads{locality#*/total}/idle-rate >> ${results_dir}/alsphx_${th}th_itrscs_${it}_${f}_${rs}_${cs}
